...
export default function Plots(){
    ...
    console.log(userFarmInfo);

    
    useEffect(()=>{
        if(user && token){
            async function fetch(){ 
                const response = await api.post('/user-farms',user, {headers: {Authorization: `Bearer ${token}`}});
                setUserFarmInfo(response.data);
            }
            fetch();            
        }
    },[user, token])
    ...
    async function handleSelectSeed(seed){
        try{
            ...
            const response1 = await api.post('/user-farms',user, {headers: {Authorization: `Bearer ${token}`}});
            const response3 = await api.post('/user-info',user, {headers: {Authorization: `Bearer ${token}`}});
            
            setUserInfo(response3.data);
            setUserFarmInfo(response1.data);
            setSelectedPlot(-1);
        ...
        <div className='plots'>
            ...
            <div className='pop-up-culivation'>
                <button id="closeButt" onClick={()=>{ document.getElementsByClassName('pop-up-culivation')[0].style.display ='none'}}>X</button>
                {
                    ...
                    userInfo?.userInventory?.map( seed => 
